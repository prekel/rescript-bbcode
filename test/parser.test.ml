...
let () =
  zoraBlock "textparser test" (fun t ->
      let open Parse in
      let open Opal in
      let txt = "123456789012345678901234567" in
      let a1 = run txt (many text_parser) in
      t |. equal a1 (Some [ Text "1234567890"; Text "1234567890"; Text "1234567" ]) "")
;;
...
let () =
  zoraBlock "many' test" (fun t ->
      let open Parse in
      let open Opal in
      let txt = "12345678901234567890" in
      let a1 = run txt (many' Opal.digit 10 => implode) in
      t |. equal a1 (Some "1234567890") "";
      let txt = "12345" in
      let a1 = run txt (many' Opal.digit 10 => implode) in
