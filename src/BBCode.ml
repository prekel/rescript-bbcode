...
let rec traverse (f : ast -> ast) =
  let fix_ast a = a |> f |> List.map (traverse f) in
  function
  | Text txt -> Text txt
  ...
  | QuoteNamed { children; quote } -> QuoteNamed { children = fix_ast children; quote }
  | Spoiler { children } -> Spoiler { children = fix_ast children }
  | SpoilerNamed { children; name } -> SpoilerNamed { children = fix_ast children; name }
  | Link { url } -> Link { url }
